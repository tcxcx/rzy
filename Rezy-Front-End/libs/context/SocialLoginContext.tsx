...
import { activeChainId } from '../../config/chainConfig';

const SocialLoginWithNoSSR = dynamic(
  () => import('@biconomy/web3-auth').then((mod) => mod.default),
  {
    ssr: false,
  },
);
...
interface web3AuthContextType {
  connect: () => Promise<SocialLoginWithNoSSR | null | undefined>;
...
export const Web3AuthProvider = ({ children }: any) => {
  ...
  const [socialLoginSDK, setSocialLoginSDK] =
    useState<SocialLoginWithNoSSR | null>(null);

  // create socialLoginSDK and call the init
  useEffect(() => {
    const initWallet = async () => {
      const sdk = new SocialLoginWithNoSSR();
      await sdk.init({
